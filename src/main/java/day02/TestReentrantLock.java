...
public class TestReentrantLock {
    static final MyReentrantLock LOCK = new MyReentrantLock(true); //true公平锁   false非公平锁
    ...
    private static void fairVsUnfair() throws InterruptedException {
        ...
        //tryLock(long timeout, TimeUnit unit)根据设置的布尔参数，遵循公平锁(true)和非公平锁(false)原则；获取不到锁，返回false,不会像t3，t4一直等待，避免不断创建线程把CPU跑满
        //tryLock() 底层是非公平锁(源码)
        while (!stop) {
            new Thread(() -> {
                try {
